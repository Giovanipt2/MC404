...
/*Transforma a instrução para um número inteiro

Parâmetro:
data -- ponteiro para a struct InstData

Retorno:
número inteiro que representa a instrução
*/
int inst_pra_int(InstData *data){
    int num = 0;

    pack(data->opcode, 0, 6, &num);

    switch (data->type){
        case R:
            pack(data->rd, 7, 11, &num);
            pack(data->funct3, 12, 14, &num);
            pack(data->rs1, 15, 19, &num);
            pack(data->rs2, 20, 24, &num);
            pack(data->funct7, 25, 31, &num);
            break;
        ...
        case I:
            ...
            pack(data->funct3, 12, 14, &num);
            pack(data->rs1, 15, 19, &num);
            pack(data->imm, 20, 31, &num);
            break;

        case S:
            pack(data->imm, 7, 11, &num);
            pack(data->funct3, 12, 14, &num);
            pack(data->rs1, 15, 19, &num);
            pack(data->rs2, 20, 24, &num);
            pack(data->imm >> 5, 25, 31, &num);
            break;

        case B:
            pack(data->imm >> 11, 7, 7, &num);
            pack(data->imm >> 1, 8, 11, &num);
            pack(data->funct3, 12, 14, &num);
            pack(data->rs1, 15, 19, &num);
            pack(data->rs2, 20, 24, &num);
            pack(data->imm >> 5, 25, 30, &num);
            pack(data->imm >> 12, 31, 31, &num);
            break;

        case U:
            pack(data->rd, 7, 11, &num);
            pack(data->imm, 12, 31, &num);
            break;

        case J:
            pack(data->rd, 7, 11, &num);
            pack(data->imm >> 12, 12, 19, &num);
            pack(data->imm >> 11, 20, 20, &num);
            pack(data->imm >> 1, 21, 30, &num);
            pack(data->imm >> 20, 31, 31, &num);
    }
...
int main(){
    ...
    int instrucao = inst_pra_int(&data);
    hex_code(instrucao);
